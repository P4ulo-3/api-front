...
export default function EditForm() {
  ...
  function handleFormCadastro(event) {
  ...
  async function getidForm() {
    try {
      const editResponse = await fetch(`http://localhost:3000/clientes/${id}`, {
        method: "GET",
        headers: {
      ...
      if (editResponse.ok) {
        const dataRes = await editResponse.json();
        setEditForm({
          nome: dataRes.nome,
          telefone: dataRes.telefone,
          email: dataRes.email,
          departamento_id: dataRes.departamento_id,
        });
      }
    } catch (error) {
      toast.error("Error na requisição");
    }
  }
  ...
  async function fetchCadastroSubmit(event) {
    ...
    try {
      const response = await fetch(`http://localhost:3000/clientes/${id}`, {
        ...
        body: JSON.stringify({
          ...
          departamento_id: editForm.departamento_id,
        }),
      });
    ...
    <main className="w-full flex">
      ...
      <div className="flex-1 flex items-center justify-center h-screen">
        <div className="w-full max-w-md space-y-8 px-4 bg-white text-gray-600 sm:px-0">
          ...
          <form onSubmit={fetchCadastroSubmit} className="space-y-5">
            <div>
